...
<u>Scenario</u>: So you averted a embarassing situation with the help of Dynatrace when you tested the new `carts` service release... and quickly found the issue.

But now, it's the turn of the marketing department to strong-arm you. :mechanical_arm:

![promo](../../assets/images/promo_socks.png)

The marketing team has been working on a promo campaign, again involving a new `carts` service release.

Since promo campaign is already late on its schedule the `carts` service has been duct-taped and the project fast-tracked to be deployed in production. This time your requests for more testing time in the dev are rejected!
...
The Kubernetes platform team has recently deployed <i>Istio</i>, a service mesh, on your Kubernetes cluster. Istio provides dynamic traffic management capabilities, without any code change. 
...
The next command will configure Istio <i>virtual routes</i> that are implementing the traffic management rules via the <i>Envoy</i> proxy sidecar containers running in each pod of the `sockshop-production` namespace.
...
![your_very_own_dashboard-4](../../assets/images/your_very_own_dashboard-4.png)
...
After a few minutes, problems should show up in the Problems feed. Switch to the new feed.
...
- There might one or multiple problems displayed, depending on the problems distinct evolution. Eventually (after a few minutes), the DAVIS AI will figure out those distinct problems are related and they will be grouped into a single one, with a root cause analysis provided. <b>(2)</b>
...
Also, remember you built "your very own dashboard" with a part of it to keep an eye on carts v1 vs v2? The devops team was looking at it during the launch and they've seen what was going on at the same time they were receiving the alert.

- <b>Menu -> Dashboards</b>

![your_very_own_dashboard-promo-problem](../../assets/images/your_very_own_dashboard-promo-problem.png)
...
- Repeat the same steps without a log in
  - When you hit the <b>Add to cart</b> button, nothing will happen. The cart status button will keep showing `0 items in the cart`, whatever the amount of time your are trying to add the item in the cart.
...
- You then have access to a rotation of 3 screen displays
  ...
  - Actual display (taken from this failing test executions)
...
### Getting visibility on the Envoy proxies

Istio is leveraging <i>Envoy</b> as a proxy server for ingress/egress control and in-cluster dynamic traffic management. <b>Envoys</b> are injected as sidecar container running in the same pod as your microservice. This is why for Istio-injected namespaces and pod, such as our `sockshop-production`, we can see 2 containers per pod (3 in the specific case of `carts-db` because there is also the MongoDB Prometheus exporter which runs as another sidecar).
...
Luckily, more robots :robot: than real users were affected. Put let's not push our luck... Because we have a service mesh, we can dynamically redirect the traffic to the stable release and everything will back to normal (except no more promo).
